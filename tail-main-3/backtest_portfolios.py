...
Reuses gen_<model>.npy from a run_baselines.py run
plus the tailfm windows, evaluates empirical h-step VaR per portfolio per
model, and backtests against the held-out period.

    python backtest_portfolios.py --data returns.csv --n 24 \
        --gen-dir baseline_out --tailfm-gen ../tailfm_pkg-3-3/run_out/generated_windows.npy
...
from tailfm import kupiec_test, make_windows, portfolio_losses
...
def parse_args():
    ...
    ap.add_argument("--log", type=str, default=None,
                    help="text file receiving a copy of everything printed "
                         "(default: {gen_dir}/backtest_report.log)")
    args = ap.parse_args()
...
def main():
    args = parse_args()
    log_path = args.log or f"{args.gen_dir}/backtest_report.log"
    with tee_output(log_path, header="backtest_portfolios.py"):
        run(args)
